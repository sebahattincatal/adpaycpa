...
$result = $mysqli->query( "SELECT COUNT(id) AS count FROM `offers`" );
?>

<p>
	<b><?php echo $loc['offers.php']['t02']; ?>&nbsp;</b>[<a href="./offers.php?addoffer=ok"><?php echo $loc['offers.php']['t150']; ?></a>]
</p>
...
<p>
	<table class="stats_table" style="width: 100%;">
		<tr class="table_zagolovki">
			...
			<td width="110"><a href="offers.php?sort_cena=ok" class="normal_link"><?php echo $loc['offers.php']['t156']; ?></a></td>		
			<td width="110"><a href="offers.php?sort_comission=ok" class="normal_link"><?php echo $loc['offers.php']['t157']; ?></a></td>
			<td width="140"><?php echo $loc['offers.php']['t158']; ?></td>
			<td width="15"><a href="offers.php?sort_status=ok" class="normal_link"><?php echo $loc['offers.php']['t159']; ?></a></td>
	...
	if (isset($_GET['sort_status']) && $_GET['sort_status']=='ok') 
		...
		if (isset($offset) && isset($show_pages))
			{
			$sql = "SELECT * FROM offers ORDER BY `active` DESC LIMIT $offset, $show_pages";
			}
		else
			{
			$sql = "SELECT * FROM offers ORDER BY `active` DESC";
			}
		}
	elseif (isset($_GET['sort_comission']) && $_GET['sort_comission']=='ok') 
		...
		if (isset($offset) && isset($show_pages))
			{
			$sql = "SELECT * FROM offers ORDER BY `comission` DESC LIMIT $offset, $show_pages";
			}
		else
			{
			$sql = "SELECT * FROM offers ORDER BY `comission` DESC";
			}
		}
	elseif (isset($_GET['sort_cena']) && $_GET['sort_cena']=='ok') 
		...
		if (isset($offset) && isset($show_pages))
			{
			$sql = "SELECT * FROM offers ORDER BY `cena` DESC LIMIT $offset, $show_pages";
			}
		else
			{
			$sql = "SELECT * FROM offers ORDER BY `cena` DESC";
			}
		}
	elseif (isset($_GET['sort_id']) && $_GET['sort_id']=='ok') 
		...
		if (isset($offset) && isset($show_pages))
			{
			$sql = "SELECT * FROM offers ORDER BY `id` DESC LIMIT $offset, $show_pages";
			}
		else
			{
				...
				<tr class="<?php if ( $cvet & 1 ) {echo 'col2';} else {echo '';} $cvet++; ?>">
					<td width="500">
						...
						<div style="text-align: left; margin-top: 8px; line-height: 21px;">
							...
							$owner_id=htmlentities($res['owner_id']);
							$sql2 = "SELECT email,balance FROM users WHERE `id`='$owner_id'";
							$result2 = $mysqli->query($sql2);
							$res2=mysqli_fetch_array($result2);
						
							// Получаем email рекламодателя
							// Get email advertiser
							$owner_email=htmlentities($res2['email']);
							
							// Получаем баланс рекламодателя
							// Get the advertiser balance
							$owner_balance=htmlentities($res2['balance']);
						
							?><?php echo $loc['offers.php']['t215']; ?>&nbsp;<a href="./users.php?edit=<?php echo $owner_id; ?>" target="_blank"><?php echo $owner_email; ?></a>&nbsp;<a target="_blank" href="./tickets.php?komu=<?php echo $owner_id; ?>"><span class="send_ticket"></span></a>
							<br />
							<?php echo $loc['offers.php']['t216']; ?>&nbsp;<?php echo $owner_balance; ?>&nbsp;<?php echo $loc['offers.php']['t17']; ?>&nbsp;
							<br />
							<?php
					...
					<td width="110" style="line-height: 18px;"><?php if (isset($offer_cena) && $offer_cena!='0') {echo $offer_cena.'&nbsp;'.$loc['offers.php']['t17'].'&nbsp;';} else {?>&nbsp;<?php echo $loc['offers.php']['t29']; ?>&nbsp;<?php } ?></td>	
					<td width="110">
						<?php 
					...
					<?php
					if ($offer_active=="2") {?><td style="width: 15px; background: yellow;"></td><?} 
					if ($offer_active=="1") {?><td style="width: 15px; background: green;"></td><?} 
					if ($offer_active=="0") {?><td style="width: 15px; background: red;"></td><?}
					?>					
				</tr>
			</table>
			<?php
