			...
			<p>
				<span>
					...
					<select name="offer" style="text-align: left; width: 300px; height: 25px;">
						...
						$offers = get_offers_rekl($user_id);
						if ($offers!=null)
							{
...
<p>
	<table class="stats_table" width="100%">
		...
		<tr style="background: #e1ecf5;">
			...
			<td colspan="4"><?php echo $loc['stats.php']['t31']; ?></td>
		...
		<?php  
		$recs = array();
		$recs[$dat2] = array('hosts'=>0, 'visits' => 0, 'zakaz_ok' => 0, 'zakaz_hold' => 0, 'zakaz_wait' => 0, 'zakaz_cancel' => 0, 'cena_ok' => 0, 'cena_hold' => 0, 'cena_wait' => 0, 'cena_cancel' => 0);
		if (isset($tdat1) && isset($tdat2)) 
			...
			for ($i = $tdat2; $i>=$tdat1; $i--) 
				...
				$recs[$j] = array('hosts'=>0, 'visits' => 0, 'zakaz_ok' => 0, 'zakaz_hold' => 0, 'zakaz_wait' => 0, 'zakaz_cancel' => 0, 'cena_ok' => 0, 'cena_hold' => 0, 'cena_wait' => 0, 'cena_cancel' => 0);
				}
			} 
		...
		foreach ( $recs as $date => $rec ) 
			...
			$cena_ok=htmlentities($rec['cena_ok']);
			$cena_hold=htmlentities($rec['cena_hold']);
			$cena_wait=htmlentities($rec['cena_wait']);
			$cena_cancel=htmlentities($rec['cena_cancel']);	
			...
			$cena_ok_all+=$cena_ok;
			$cena_hold_all+=$cena_hold;
			$cena_wait_all+=$cena_wait;
			$cena_cancel_all+=$cena_cancel;
			...
			<tr class="<?php if ( $cvet & 1 ) {echo 'col2';} else {echo '';} $cvet++; ?>">
				...
				<td><nobr><?php echo $cena_ok; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
				<td><nobr><?php echo $cena_hold; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
				<td><nobr><?php echo $cena_wait; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
				<td><nobr><?php echo $cena_cancel; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
				<td><nobr><?php if ($hosts!='0') {echo round($zakaz_ok/$hosts*100, 0);} else {echo '0';} ?></nobr></td>
				<td><nobr><?php if ($hosts!='0') {echo round($cena_ok/$hosts, 0);} else {echo '0';} ?></nobr></td>
			</tr>
			<?php
			}  
		?>	
		...
		<tr style="background: #e1ecf5;">
			...
			<td width="85"><nobr><?php echo $cena_ok_all; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
			<td width="85"><nobr><?php echo $cena_hold_all; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
			<td width="85"><nobr><?php echo $cena_wait_all; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
			<td width="85"><nobr><?php echo $cena_cancel_all; ?>&nbsp;<?php echo $loc['stats.php']['t26']; ?>&nbsp;</nobr></td>
			<td width="50"><nobr><?php if ($hosts_all!='0'){echo round($zakaz_ok_all/$hosts_all*100, 0);} else {echo '0';}?></nobr></td>
			<td width="50"><nobr><?php if ($hosts_all!='0'){echo round($cena_ok_all/$hosts_all, 0);} else {echo '0';}?></nobr></td>		
		</tr>  		
	</table>
</p>
