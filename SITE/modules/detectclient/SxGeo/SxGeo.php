...
class SxGeo {
	...
	public function __construct($db_file = '/SxGeoCity.dat', $type = SXGEO_FILE){
		...
		// First, we see that there is a database file
		$header = fread($this->fh, 40); // В версии 2.2 заголовок увеличился на 8 байт // In version 2.2, the title has increased by 8 bytes //
	...
	public function get_num($ip){
		$ip1n = (int)$ip; // Первый байт // The first byte //
		...
		// Find the data block in the index of the first byte
		if ($this->batch_mode){
		...
		if ($blocks['max'] - $blocks['min'] > $this->range){
			...
			// We are looking for a block in the main index
			...
			// Found a block number in which you want to search for the IP, is now in the desired unit in the database
			...
			// It is necessary to check that the unit did not go beyond the first byte block
